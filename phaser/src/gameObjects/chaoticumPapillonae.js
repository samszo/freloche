...
export default class chaoticumPapillonae {
    constructor(params) {
        var me = this;
        ...
        function setWings(){
            ...
            wingR = svg.append("g").attr('id','wingR')
                .attr("transform","matrix(-1 0 0 1 "+(2*(posis.head.cx))+" 0)"),
            modele = me.modelesWing[Math.floor(Math.random()*me.modelesWing.length)];

            //charger le modèle d'aile en synchrone
            const request = new XMLHttpRequest();
            request.overrideMimeType("text/xml");
            request.open('GET', modele, false);  
            request.send(null);        
            if (request.status === 200) {
                ...
                wingL.select(q).attr('width',size/2).attr('height',size);
                wingR.node().appendChild(modeleWingR);
                wingR.select(q).attr('width',size/2).attr('height',size);
                //calcule les dégradés pour chaque élément
                wingL.select(q).selectAll('path').each(setPathDegrad);
                wingL.select(q).selectAll('ellipse').each(setPathDegrad);
                wingL.select(q).selectAll('circle').each(setPathDegrad);
                wingR.select(q).selectAll('path').each(setPathDegrad);
                wingR.select(q).selectAll('ellipse').each(setPathDegrad);
                wingR.select(q).selectAll('circle').each(setPathDegrad);
            }
        }
        function setPathDegrad(e,d){
            ...
            if(!bb.x)bb.x=10;
            if(!bb.y)bb.y=10;
            if(!bb.height)bb.height=100;
            if(!bb.width)bb.width=100;
